...
export function run(creep: Creep): TaskStatus {
    ...
    const storages: StructureStorage[] = _.sortByOrder(creep.room.find(FIND_MY_STRUCTURES, {
        filter: s => (s.structureType === STRUCTURE_SPAWN ||
                s.structureType === STRUCTURE_EXTENSION ||
                s.structureType === STRUCTURE_STORAGE ||
                s.structureType === STRUCTURE_TOWER) &&
                (s as StructureStorage).store.getFreeCapacity(RESOURCE_ENERGY) > 0
    }), s => s.store.getFreeCapacity(RESOURCE_ENERGY) && creep.pos.getRangeTo(s), ["asc", "asc"]);
